...
public class ItemSetChooser {
	...
	private void closure() {
		...
		if (selected.size() == 0) {
			JFLAPError.show("You must select an item (or items) in the right table.", "Nothing Selected");
			return;
		}
	...
	private void addItem(Production item) {
		if (restricted != null && !restricted.contains(item)) {
			JFLAPError.show(item.toString()+ " is not part of the set.", "Item not Desirable");
			return;
		}
		if (alreadyChosen.contains(item)) {
			JFLAPError.show(item.toString()+ " is already chosen.", "Item Already Chosen");
			return;
		}
