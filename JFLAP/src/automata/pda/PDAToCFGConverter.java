...
public class PDAToCFGConverter {
	...
	public Production getSimplifiedProduction(Production production) {
		...
		while ((leftIndex = rhs.indexOf(PDAToCFGConverter.LEFT_PAREN)) != -1
				&& (rightIndex = rhs.indexOf(PDAToCFGConverter.RIGHT_PAREN)) != -1) {
	...
	public ContextFreeGrammar convertToContextFreeGrammar(Automaton automaton) {
		...
		while (it.hasNext()) {
			...
			grammar.isValidProduction(getSimplifiedProduction(p));
		}
	...
	public void purgeProductions(Automaton automaton, GrammarTableModel model) {		
		...
		while (mapIter.hasNext() && freeIter.hasNext()) {			
			key = (String) mapIter.next();
			if (((String)MAP.get(key)).charAt(0) > BOTTOM_OF_STACK.charAt(0))
	...
	private static final Object START_VAR = "S";
